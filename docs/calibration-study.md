...
`.gitignore`, so they are not committed. The README table is the artifact of record;
the pair rows inside the JSON are what the disagreement analyses in
`design-notes.md` §6 are computed from.
